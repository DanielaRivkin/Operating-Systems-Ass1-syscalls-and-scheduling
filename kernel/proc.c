...
exit(int status, char* exit_msg)
{
  ...
  safestrcpy(p->exit_msg, exit_msg, 32); // task 3
...
long long get_min(int limit) {
  struct proc *proc_ptr = proc;
  long long min_val = 0;
  int run_count = 0;
  while (proc_ptr != &proc[64]) {
    if (proc_ptr->state == RUNNABLE || proc_ptr->state == RUNNING) {
      if (run_count == 0 || proc_ptr->accumulator < min_val) {
        min_val = proc_ptr->accumulator;
      }
      run_count++;
    }
    proc_ptr++;
...
void scheduler(void){
  ...
  for(;;){
    if (policy == 0) {
          ...
          int decay;
          int rightside;
          int funinder = __INT32_MAX__;;
          
          for(p = proc; p < &proc[64]; p++, i++) {
            if(p->state == RUNNABLE) {
                if(!(p->rtime != 0 ||
                   p->stime != 0 || 
                   p->retime!= 0)){
                ...
                decay = (75 + (25 * p->ps_priority));
                rightside = (p->rtime/(p->rtime + p->stime + p->retime));
                vruntime = (decay * rightside);

                if(vruntime == 0){
                  pnum = i;
                  break;
                }
                else if(vruntime < funinder){
                  pnum = i;
                  funinder = vruntime;
                }
            }
...
void set_policy(uint64 n){
    policy = n;
}

void update_count() {
  struct proc *p_ptr;
  for (p_ptr = proc; p_ptr < &proc[64]; p_ptr++) {
    acquire(&p_ptr->lock);
    if (p_ptr->state == SLEEPING) {
      p_ptr->stime++;
    } 
    else if (p_ptr->state == RUNNABLE) {
      p_ptr->retime++;
    } 
...
wakeup(void *chan)
{
  ...
  long long min = get_min(0); 		//task5 
  for(p = proc; p < &proc[NPROC]; p++) {
    if(p != myproc()){
      acquire(&p->lock);
      if(p->state == SLEEPING && p->chan == chan) {
        p->state = RUNNABLE;
        p->accumulator = min; 					//task5 
      }
      release(&p->lock);
    }
  }
